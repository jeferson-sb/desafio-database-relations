...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (!existingProducts.length) {
      throw new AppError('Cound not find any products with given id');
    }
    ...
    const findProductsWithNoQtyAvailable = products.filter(product => {
      return (
    ...
    const orderedProductsQty = products.map(product => ({
      id: product.id,
      quantity:
        existingProducts.filter(p => p.id === product.id)[0].quantity -
        product.quantity,
